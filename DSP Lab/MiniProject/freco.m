...
clear all
clc
cd 'C:\Users\hemant\Documents\Engineering\SEM 4\DSP Lab\MiniProject\ORL'
...
for NTr = 1 : 3;    % change number of training images 'NTr' per person
...
for itr=1:size(rndMtrx,1)  % run the program randomly 10 times then average results.
    ...
    for i=1:NP
        for j=1:NTr
            ...
            %1D array of images
            [X,MAP]=imread(im);
            %map is a container in MATLAB
            x=ind2gray(X,MAP);
            %https://in.mathworks.com/help/images/ref/ind2gray.html
            %I = ind2gray(X,map) converts the image X with colormap map to a grayscale image I. 
            %ind2gray removes the hue and saturation information from the input image while retaining the luminance.
